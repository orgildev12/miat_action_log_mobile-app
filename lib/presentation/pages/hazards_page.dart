            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('User ID: ${hazard.userId}'),
                  Text('User Name: ${hazard.userName ?? "No user name provided"}'),
                  Text('Email: ${hazard.email ?? "No email provided"}'),
                  Text('Phone Number: ${hazard.phoneNumber ?? "No phone number provided"}'),
                  Text('Type ID: ${hazard.typeId}'),
                  Text('Type Name EN: ${hazard.typeNameEn}'),
                  Text('Type Name MN: ${hazard.typeNameMn}'),
                  Text('Location ID: ${hazard.locationId}'),
                  Text('Location Name EN: ${hazard.locationNameEn}'),
                  Text('Location Name MN: ${hazard.locationNameMn}'),
                  Text('Description: ${hazard.description}'),
                  Text('Solution: ${hazard.solution}'),
                  Text('Is Private: ${hazard.isPrivate}'),
                  Text('Date Created: ${hazard.dateCreated}'),
                  const SizedBox(height: 12),
                  // Response Information
                  Text('RESPONSE DATA:', 
                       style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.blue)),
                  const SizedBox(height: 4),
                  if (hazard.responseOfHazard != null) ...[
                    Text('Response Started: ${hazard.responseOfHazard!.isStarted == 1 ? "YES" : "NO"}'),
                    Text('Response Body: ${hazard.responseOfHazard!.responseBody ?? "No response body"}'),
                    Text('Request Approved: ${hazard.responseOfHazard!.isRequestApproved == 1 ? "YES" : "NO"}'),
                    Text('Response Confirmed: ${hazard.responseOfHazard!.isResponseConfirmed == 1 ? "YES" : "NO"}'),
                    Text('Response Updated: ${hazard.responseOfHazard!.dateUpdated}'),
                  ] else ...[
                    Text('❌ ERROR: No response found for Hazard ID ${hazard.id}'),
                    Text('This should not happen - every hazard should have a response'),
                    Text('Check backend /response/${hazard.id} endpoint'),
                  ],
                  const SizedBox(height: 8),
                  Text('ALL HAZARD PROPERTIES:', 
